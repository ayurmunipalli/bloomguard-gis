...
- **Splits.** Three: random, temporal (train earlier years, test later), and spatial (hold out counties/blocks). The **temporal** split is the honest test of forecasting skill. Grouped/blocked splits prevent spatially adjacent cell-days from leaking across train/test.
  - `NOTE(paper):` **Spatial-generalization claim corrected post-P0-B (see PROJECT.md §2.1).** The block holdout alone did **not** prevent adjacency leakage — 14.6% of test cells lay within ~10 km of a train cell. After the P0-B spatial buffer, geographic transfer (spatial H=7 PR-AUC **0.617**) sits *below* random resampling (**0.631**); the earlier spatial>random gap (0.663) was border leakage. **Frame as: "geographic transfer to a held-out region is harder than random resampling; earlier apparent spatial robustness was inflated by border adjacency, now corrected."** This is the expected direction (unseen geography is harder), so it is a strengthened, trustworthy result — not a regression. Do not describe the model as "generalizing well spatially."
